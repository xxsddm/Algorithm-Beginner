...
class Solution:             # 只有python中这种方法比较快
    def unhappyFriends(self, n: int, preferences: List[List[int]], pairs: List[List[int]]) -> int:
        ...
        for pair in pairs:
            for idx in range(2):
                person = pair[idx]  # person: 当前目标人

                # preference: person的好感列表; pair[1 - idx]: person当前匹配的人
                preference = preferences[person]

                for other in preference:        # other: person 好感列表的其他人
                    if other == pair[1 - idx]:
                        break
                    if preferences[other].index(container[other]) > preferences[other].index(person):
                        ans += 1
                        break
        return ans
