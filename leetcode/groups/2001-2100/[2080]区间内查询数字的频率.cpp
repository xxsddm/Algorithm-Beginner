...
public:
    ...
    int query(int left, int right, int value) {
        if (nums[value].empty() || nums[value][0] > right) {
            return 0;
        }
        int idxRight = bisect(nums[value], right, -1);
        int idxLeft = left < nums[value][0] ? -1 : bisect(nums[value], left - 1, idxRight);
        return idxRight - idxLeft;
    }

    int bisect(vector<int> &arr, int target, int limit) {   // 二分
        int left = 0, right = limit > 0 ? limit : ((int) arr.size() - 1);
        while (left <= right) {
