...
public:
    ...
    int countPyramids(vector<vector<int>>& grid) {  // DP
    ...
    void calculate() {
        vector<int> counter(numCol, 0), temp(numCol, 0);
        for (int row = numRow - 2; row >= 0; row--) {   // 枚举尖端位置
            for (int col = 1; col <= numCol - 2; col++) {
                if (nums[row][col] &&
                        ...
                        nums[row + 1][col] &&
                        nums[row + 1][col + 1]) {
                    temp[col] = 1 + min(counter[col - 1], min(counter[col], counter[col + 1]));
                    ans += temp[col];
                } else {
                    temp[col] = 0;
                }
