...
// 
// Related Topics 广度优先搜索 数组 矩阵 👍 85 👎 0


//leetcode submit region begin(Prohibit modification and deletion)
class Solution {
public:
    int numRow, numCol, dest;

    int snakesAndLadders(vector<vector<int>>& board) {  // BFS
        numRow = (int) board.size(), numCol = (int) board[0].size(), dest = numRow * numCol;
        int ans = 0;
        bool visited[dest];
        queue<int> container;
        ...
        while (!container.empty()) {
            ...
            for (int i = 0, size = (int) container.size(); i < size; i++) {
                ...
                container.pop();
                for (int next = cur + 1, limit = min(cur + 6, dest); next <= limit; next++) {
                    pair<int, int> rc = loc2rc(next);
                    int temp = board[rc.first][rc.second];
                    if (temp == -1) {
                        temp = next;
                    }
                    if (temp == dest) {
    ...
    pair<int, int> loc2rc(int loc) {
        int rowPassed = --loc / numCol, colPassed = loc % numCol;
        if (rowPassed & 1) {
            return {numRow - 1 - rowPassed, numCol - 1 - colPassed};
        }
        return {numRow - 1 - rowPassed, colPassed};
    }
};
