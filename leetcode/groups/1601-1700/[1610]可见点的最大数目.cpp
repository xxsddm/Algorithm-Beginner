...
public:
    int visiblePoints(vector<vector<int>>& points, int angle, vector<int>& location) {
        ...
        for (vector<int> &point : points) {
            if (point != location) {
                // atan2 -> [-pi,pi] 按极角排序
                container.push_back(atan2(point[1] - location[1], point[0] - location[0]));
            } else {
                numSame++;
            }
        }
        ...
        length <<= 1;
        double limit = (double) angle / 180 * M_PI + 1e-6;
        while (end < length) {  // 滑动窗口
            double next = end >= prevLen ? container[end - prevLen] + 2 * M_PI : container[end];
            if (next - container[start] <= limit) {
                ans = max(ans, end++ - start + 1 + numSame);
