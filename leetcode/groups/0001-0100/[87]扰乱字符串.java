...
class Solution {
    public boolean isScramble(String s1, String s2) {   // 区间DP
        ...
        for (int start1 = length - 1; start1 >= 0; start1--) {
            for (int start2 = length - 1; start2 >= 0; start2--) {
                for (int len = 2; len <= length - Math.max(start1, start2); len++) {
...
class Solution {
    ...
    public boolean isScramble(String s1, String s2) {   // 记忆化DP(参考官方题解)
        word1 = s1;
        word2 = s2;
    ...
    private boolean dfs(int start1, int start2, int length) {
        // 排除可以直接判断的情况
        if (dp[start1][start2][length] != 0) {
            return dp[start1][start2][length] == 1;
        }
        if (word1.substring(start1, start1 + length).equals(
                word2.substring(start2, start2 + length))) {
    ...
    private boolean check(int start1, int start2, int length) {
