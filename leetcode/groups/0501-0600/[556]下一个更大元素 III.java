...
class Solution {
    public int nextGreaterElement(int n) {
        ...
        int[] container = new int[String.valueOf(n).length()];
        for (int i = container.length - 1; i >= 0; i--) {
            container[i] = n % 10;
            n /= 10;
        }
        int idx = container.length - 2;
        while (idx >= 0) {
        ...
        int left = idx + 1, right = container.length - 1;
        while (left < right) {
            temp = container[left];
        ...
        // 可以不用long
        int ans = 0, limit = Integer.MAX_VALUE;
    ...
    private int bisect(int idx, int[] container) {  // 二分搜索比idx元素大的idx最右侧数字
