...
class Solution {
    public int[] findRedundantDirectedConnection(int[][] edges) {
        // 记录各节点入度
        int n = edges.length, mark = 0;
        int[] indegree = new int[n + 1];
        // 保存从节点出发的有向边的指向
        LinkedList<Integer>[] container = (LinkedList<Integer>[]) new LinkedList[n + 1];
        for (int[] edge: edges) {
            if (container[edge[0]] == null) {
                container[edge[0]] = new LinkedList<>();
            }
            indegree[edge[1]]++;
            container[edge[0]].add(edge[1]);
    ...
    boolean dfs(int node, int ban, int end, List<Integer>[] container) {
