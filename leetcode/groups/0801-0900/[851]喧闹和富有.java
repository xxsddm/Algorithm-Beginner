...
// 
// Related Topics 深度优先搜索 图 拓扑排序 数组 👍 182 👎 0
...
class Solution {
    ...
    public int[] loudAndRich(int[][] richer, int[] quiet) { // DFS
        int length = quiet.length;
        ans = new int[length];
        for (int i = 0; i < length; i++) {
            ans[i] = -1;
        }
        next= (LinkedList<Integer>[]) new LinkedList[length];
        for (int[] pair : richer) {
            if (next[pair[1]] == null) {
    ...
    int dfs(int node, int[] quiet) {
        ...
        for (int nextNode : next[node]) {
            if (quiet[temp] > quiet[dfs(nextNode, quiet)]) {
                temp = ans[nextNode];
            }
        }
        ans[node] = temp;
