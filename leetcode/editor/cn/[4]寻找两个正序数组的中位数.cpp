...
// Related Topics 数组 二分查找 分治 👍 4603 👎 0


//leetcode submit region begin(Prohibit modification and deletion)
class Solution {
public:
    double findMedianSortedArrays(vector<int>& nums1, vector<int>& nums2) { // 双指针
        int length = (int) (nums1.size() + nums2.size());
        if ((length & 1) != 0) {
            return findKth(nums1, nums2, (length >> 1) + 1);
        }
        return (findKth(nums1, nums2, length >> 1)
                + findKth(nums1, nums2, (length >> 1) + 1)) / 2.0;
    }
    ...
    int findKth(vector<int>& nums1, vector<int>& nums2, int k) {
        ...
        while (true) {
            ...
            int nextIdx1 = min(candidate1, length1), pivot1 = nums1[nextIdx1 - 1];
            int nextIdx2 = min(candidate2, length2), pivot2 = nums2[nextIdx2 - 1];
