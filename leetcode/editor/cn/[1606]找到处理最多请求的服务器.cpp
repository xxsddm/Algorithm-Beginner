...
// arrival 保证 严格递增 。 
// 
// Related Topics 贪心 数组 有序集合 堆（优先队列） 👍 24 👎 0
...
public:
    vector<int> busiestServers(int k, vector<int>& arrival, vector<int>& load) {
        int length = (int) arrival.size(), maxCount = 0, counter[k];
        priority_queue<pair<int, int>, vector<pair<int, int>>, greater<>> pq;   // 当前运行<结束时间,服务器索引(经修改)>
        priority_queue<int, vector<int>, greater<>> available;  // 当前可用服务器索引(经修改)
        memset(counter, 0, sizeof(counter));
        for (int i = 0; i < k; i++) {
            available.push(i);  // 最初k个任务一定使用
        }
        for (int i = 0; i < length; i++) {
            ...
            int temp = available.top();
            available.pop();
            pq.push({cur + load[i], temp});
            counter[temp % k]++;    // 转换成原始服务器索引
        }
        for (int num: counter) {
