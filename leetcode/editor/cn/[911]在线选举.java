...
class TopVotedCandidate {
    ...
    public TopVotedCandidate(int[] persons, int[] times) {
        ...
        for (int i = 0, maxCount = 0, length = times.length; i < length; i++) {
            counter.put(persons[i], counter.getOrDefault(persons[i], 0) + 1);
            int temp = counter.get(persons[i]);
            if (temp >= maxCount) {
                maxCount = temp;
                container.add(new int[] {times[i], persons[i]});
            }
        }
    }
    ...
    public int q(int t) {
        ...
        while (left <= right) {
            ...
            if (container.get(mid)[0] <= t) {
                left = mid + 1;
            } else {
                right = mid - 1;
            }
        }
        return container.get(right)[1];
    }
}
