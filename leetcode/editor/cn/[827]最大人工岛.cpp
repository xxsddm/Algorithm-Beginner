...
public:
    ...
    int largestIsland(vector<vector<int>>& grid) {  // 并查集
        int ans = 0;
        numRow = (int) grid.size(), numCol = (int) grid[0].size();
        int total = numRow * numCol, root[total], size[total];
        ...
        for (int i = 0; i < numRow; i++) {
            for (int j = 0; j < numCol; j++) {
                ...
                if (grid[i][j] == 1) {
                    if (i > 0 && grid[i - 1][j] == 1) {
                        merge(root, size, idx, loc2Idx(i - 1, j));
                    }
                    if (j > 0 && grid[i][j - 1] == 1) {
                        merge(root, size, idx, loc2Idx(i, j - 1));
                    }
                }
            }
        }
        for (int i = 0; i < numRow; i++) {
            for (int j = 0; j < numCol; j++) {
                if (grid[i][j] == 1) {
                    ans = max(ans, size[find(root, loc2Idx(i, j))]);
                }
            }
        }
            ...
            for (int col = 0; col < numCol; col++) {
                ...
                for (int k = 0; k < 4; k++) { // 考虑四个相邻方向
                    ...
                    if (nextRow >= 0 && nextRow < numRow &&
                        ...
                        next.insert(find(root, loc2Idx(nextRow, nextCol)));
                    }
                }
                if (next.empty()) {
                    continue;
    ...
    void merge(int *root, int *size, int idx1, int idx2) {
        int root1 = find(root, idx1), root2 = find(root, idx2);
        if (root1 == root2) {
            return;
        }
