...
class Solution {
    public int longestCommonSubsequence(String text1, String text2) {       // DP
        // idx1+1,idx2+1(扫过的长度): 考虑text1的0~idx1, text2的0~idx2最大公共长度
        ...
        for (int idx1 = 0; idx1 < text1.length(); idx1++) {
            for (int idx2 = 0; idx2 < text2.length(); idx2++) {
                ...
                //  dp[idx1 + 1][idx2 + 1] = Math.max(
                //  dp[idx1][idx2] + (text1.charAt(idx1) == text2.charAt(idx2) ? 1 : 0),
