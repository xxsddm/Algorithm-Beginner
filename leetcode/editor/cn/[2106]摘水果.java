...
class Solution {
    public int maxTotalFruits(int[][] fruits, int startPos, int k) {
        int length = fruits.length, total = 0;
        int maxidx = fruits[length - 1][0], minidx = fruits[0][0];
        int[] cumsum = new int[maxidx + 1];
        ...
        int ans = 0;
        for (int left = Math.min(k, startPos - minidx); left >= 0; left--) {
            int right = k - left, leftidx = startPos - left, rightidx = leftidx + right;
            int temp = cumsum[Math.max(Math.min(rightidx, maxidx), startPos)] - (leftidx > 0 ? cumsum[leftidx - 1] : 0);
            ans = Math.max(ans, temp);
            if (rightidx > maxidx) {
                break;
            }
        }
        for (int right = Math.min(k, maxidx - startPos); right >= 0; right--) {
            int left = k - right, rightidx = startPos + right, leftidx = rightidx - left;
            leftidx = Math.min(startPos, leftidx);
            int templeftidx = Math.max(leftidx, minidx);
