...
public:
    ...
    int maxCompatibilitySum(vector<vector<int>>& students, vector<vector<int>>& mentors) {  // 回溯暴力破解(记忆化)
        length = (int) students.size();
        idxs = vector<int>(length);
        // 学生id,导师id -> 该对得分
        container = vector<vector<int>>(length, vector<int>(length));
