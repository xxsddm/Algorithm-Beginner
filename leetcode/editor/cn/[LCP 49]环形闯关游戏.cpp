...
class Solution {    // 优化版见java
public:
    int length;
    vector<long long> nums;

    long long ringGame(vector<long long>& challenge) {  // 从数组最大数的最高二进制位次开始枚举每一位是否必须为1
        length = (int) challenge.size();
        if (length == 1) {
            return challenge[0];
        }
        nums = challenge;
        ...
        long long maxnum = 0, ans = (long long) 1 << maxIdx;
...
//        }
    ...
    // 这里只处理了start=next+1,而prev部分没有保存,待优化(优化版本见java)
    bool check(long long num) { // 输入的num是否可以作为初始值遍历challenge
        int start = 0;
        while (start < length){
            long long curr = num;
            if (curr < nums[start]) {
            ...
            if (prev != next) {
                if (next <= start) {
                    // next可能穿过末尾从头开始,而next则已由之前较小start作为起点尝试过
    ...
    int getIdx(int idx) {   // 返回根据challenge长度修正的环形索引
        if (idx == -1) {
            return length - 1;
        }
        if (idx == length) {
