...
class Solution {
public:
    bool canVisitAllRooms(vector<vector<int>>& rooms) { // 利用并查集
        int length = (int) rooms.size(), count = 1;
        queue<int> nodes;
        vector<int> root(length);
        ...
        while (!nodes.empty()) {
            ...
            for (int i = 0; i < size; i++) {
                int room = nodes.front();
                nodes.pop();
                for (int newRoom: rooms[room]) {
