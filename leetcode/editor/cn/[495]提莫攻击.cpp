...
// 
// Related Topics 数组 模拟 👍 184 👎 0


//leetcode submit region begin(Prohibit modification and deletion)
class Solution {
public:
    int findPoisonedDuration(vector<int>& timeSeries, int duration) {   // 二分
        if (duration == 0) {
            return 0;
        }
        int ans = 0, idx = 0, length = (int) timeSeries.size(), last = timeSeries[0];
        while (idx < length) {
            ans += duration;
            last += duration - 1;
            ...
            if (idx < length) {
                // 减去下一步重复计算的部分
                ans -= max(last - timeSeries[idx] + 1, 0);
                last = timeSeries[idx];
            }
        }
        return ans;
    ...
    int bisect(vector<int>& nums, int start, int target) {
