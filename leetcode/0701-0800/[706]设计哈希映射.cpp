...
public:
    ...
    MyHashMap(): container(1009) {}

    /** value will always be non-negative. */
    void put(int key, int value) {
        int loc = hash(key);
        for (auto & pair : container[loc]) {
            if (pair.first == key) {
                pair.second = value;
                return;
            }
        }
        container[loc].push_back(make_pair(key, value));
    }
    ...
    void remove(int key) {
        int loc = hash(key);
        for (auto pair = container[loc].begin(); pair != container[loc].end(); pair++) {
            if ((*pair).first == key) {
                container[loc].erase(pair);
                return;
            }
        }
    }
