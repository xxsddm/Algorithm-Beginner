...
//consecutive-numbers/ 
// Related Topics 贪心 数组 哈希表 排序 👍 137 👎 0
...
class Solution {
    public boolean isNStraightHand(int[] hand, int groupSize) {
        ...
        for (int num : hand) {
            ...
            int size = counter.get(num);
            counter.remove(num);
            for (int i = num + 1, limit = num + groupSize; i < limit; i++) {
                if (!counter.containsKey(i) || counter.get(i) < size) {
                    return false;
                }
                counter.put(i, counter.get(i) - size);
            }
        }
        return true;
    }
