...
class Solution {
public:
    int kIncreasing(vector<int>& arr, int k) {  // LIS
        int length = (int) arr.size(), ans = 0;
        for (int start = 0; start < k; start++) {
            ...
            for (int i = start; i < length; i += k) {
                ...
                if (idx == (int) container.size()) {
                    container.push_back(arr[i]);
                } else {
                    container[idx] = arr[i];
                }
            }
            ans += count - (int) container.size();
        }
        return ans;
    }

    int bisect(vector<int> &container, int num) {
