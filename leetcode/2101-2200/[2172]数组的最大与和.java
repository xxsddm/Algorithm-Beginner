...
class Solution {
    public int maximumANDSum(int[] nums, int numSlots) {    // DP
        int length = 1, ans = 0;
        ...
        for (int num : nums) {  // 01背包
            for (int status = length - 1; status >= 1; status--) {
                for (int tempStatus = status, temp = 1, i = 1; tempStatus > 0; tempStatus /= 3, temp *= 3, i++) {
                    if (tempStatus % 3 != 0) {
                        dp[status] = Math.max(dp[status], dp[status - temp] + (num & i));
                    }
                }
            }
        }
