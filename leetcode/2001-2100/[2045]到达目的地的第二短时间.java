...
class Solution {
    public int secondMinimum(int n, int[][] edges, int time, int change) {  // BFS
        int steps = 0;
        // 是否之前已访问过n
        boolean reach = false;
        LinkedList<Integer>[] container = (LinkedList<Integer>[]) new LinkedList[n + 1];
        ...
        for (int[] edge: edges) {
            ...
            if (container[node1] == null) {
                container[node1] = new LinkedList<>();
            }
            if (container[node2] == null) {
                container[node2] = new LinkedList<>();
            }
            container[node1].add(node2);
            container[node2].add(node1);
        }
        queue.add(1);
        while (true) {
            steps++;
            ...
            for (int i = 0; i < size; i++) {
                ...
                for (int next: container[prev]) {
                    // 同一轮抵达相同位置和已访问两次的,可以省略
                    if (!visited.contains(next) && !second.contains(next)) {
                        if (next == n) {
                            if (!reach) {
                                reach = true;
                            }
    ...
    // 根据已走步数计算所需时间
    private int count(int steps, int time, int change) {
        int ans = 0, delta = change << 1;
        int lower = change, upper = lower + change - 1;
        for (int i = 0; i < steps - 1; i++) {
            ans += time;
            while (ans > upper) {
                int count = Math.max((ans - upper) / delta, 1);
                lower += count * delta;
