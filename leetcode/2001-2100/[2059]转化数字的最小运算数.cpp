...
class Solution {
public:
    int minimumOperations(vector<int>& nums, int start, int goal) { // BFS
        int count = 1;
        queue<int> visited;
        vector<bool> container(1001);
        visited.push(start);
        ...
        while (!visited.empty()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                visited.pop();
                for (int num: nums) {
                    for (int nextNum: vector<int> {temp + num, temp - num, temp ^ num}) {
                        if (nextNum == goal) {
                            return count;
                        }
                        if (nextNum >= 0 && nextNum <= 1000 && !container[nextNum]) {
                            container[nextNum] = true;
                            visited.push(nextNum);
                        }
                    }
                }
            }
