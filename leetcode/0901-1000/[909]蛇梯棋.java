...
class Solution {
    ...
    public int snakesAndLadders(int[][] board) {    // BFS
        ...
        while (!container.isEmpty()) {
            ...
            for (int i = 0, size = container.size(); i < size; i++) {
                ...
                for (int next = cur + 1, limit = Math.min(cur + 6, dest); next <= limit; next++) {
                    int[] rc = loc2rc(next);
                    int temp = board[rc[0]][rc[1]];
                    if (temp == -1) {
                        temp = next;
                    }
                    if (temp == dest) {
    ...
    int[] loc2rc(int loc) {
        int rowPassed = --loc / n, colPassed = loc % n;
        if ((rowPassed & 1) == 1) {
            return new int[] {n - 1 - rowPassed, n - 1 - colPassed};
        }
        return new int[] {n - 1 - rowPassed, colPassed};
    }
}
