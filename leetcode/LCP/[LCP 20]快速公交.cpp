...
public:
    ...
    unordered_map<uint64_t, uint64_t> container;
    ...
    uint64_t dfs(uint64_t loc) {
        ...
        if (container.find(loc) != container.end()) {
            return container[loc];
        }
        // 考虑从原点0步行到loc
        uint64_t minCost = loc * inc;

        // 考虑公交车(可能搭配步行)到loc(不一定从原点0)
        for (int bus = 0; bus < jump.size(); bus++) {
            int step = jump[bus], fare = cost[bus];
            if (loc % step == 0) {  // 刚好该位置可由某公交抵达
                // 倒推出前一站位置
                minCost = min(minCost, dfs(loc / step) + fare);
            }
            else {  // 不可由公交刚好抵达则步行剩余路程,不可能乘公交完成剩余路程,这种情况已在其他公交(bus其他取值)时考虑
                uint64_t before = loc % step, after = step - loc % step;
                minCost = min(minCost,
                              min(dfs(loc / step) + before * inc + fare,    // 在loc前下车
                                  dfs(loc / step + 1) + after * dec + fare));   // 在loc后下车
            }
        }
        container[loc] = minCost;
        return minCost;
    }
};
